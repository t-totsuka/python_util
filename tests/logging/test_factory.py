...
def test_単体正常系_get_loggerが_同一ファイルへ統合された複数ロガーに対して_単一のファイルハンドラインスタンスを共有する(
    ...
    logger_a = get_logger("myapp.module_a")
    logger_b = get_logger("myapp.module_b")
    ...
    assert file_handler_a is file_handler_b


def test_単体正常系_reset_registryが_ファイルハンドラキャッシュも破棄する(tmp_path, monkeypatch):
    log_file = tmp_path / "reset.log"
    config = LoggingConfig(file_path=log_file)
    monkeypatch.setattr(factory_module, "load_config", lambda start_dir=None: config)

    get_logger("myapp.reset_target")
    assert len(factory_module._file_handler_cache) == 1

    factory_module._reset_registry()

    assert len(factory_module._file_handler_cache) == 0
