...
def test_単体正常系_load_configが_UTF8の非ASCII文字を含む場合_ロケールに依存せず解析する(tmp_path):
...
def test_異常系_load_configが_UTF8として不正なバイト列を含む場合_警告を出しデフォルト設定にフォールバックする(tmp_path):
    (tmp_path / "pyproject.toml").write_bytes(
        "# 日本語コメント\n[tool.python_util.logging]\n".encode("cp932")
    )
...
def test_単体正常系_load_configが_rotationテーブルを受け取った場合_有効フラグと保持日数を解析する(tmp_path):
    _write(tmp_path / "pyproject.toml", """
        [tool.python_util.logging]
        level = "INFO"
...
def test_単体正常系_load_configが_rotationテーブルが存在しない場合_ローテーション有効かつ保持7日を既定とする(tmp_path):
    _write(tmp_path / "pyproject.toml", """
        [tool.python_util.logging]
        level = "INFO"
        """)
...
@pytest.mark.parametrize(
    "invalid_value",
    ["0", "-1", "true", '"7"', "7.5"],
    ids=["ゼロ", "負数", "bool値", "文字列", "浮動小数点数"],
)
def test_異常系_load_configが_retention_daysに不正値を受け取った場合_警告を出しデフォルト設定にフォールバックする(
    tmp_path, invalid_value
):
    _write(tmp_path / "pyproject.toml", f"""
        [tool.python_util.logging.rotation]
        retention_days = {invalid_value}
        """)
...
def test_異常系_load_configが_rotation_enabledに非bool値を受け取った場合_警告を出しデフォルト設定にフォールバックする(tmp_path):
    _write(tmp_path / "pyproject.toml", """
        [tool.python_util.logging.rotation]
        enabled = "yes"
        """)
