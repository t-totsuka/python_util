...
- 実行開始: 2026-07-15 07:56:47
- 実行終了: 2026-07-15 07:56:47
- 合計: 295件
- 成功: 295件
- 失敗: 0件
- エラー: 0件
...
| test_単体正常系_get_loggerが_設定レベル未満のメッセージを受け取った場合_出力から除外する[40-30] | 成功 | 0.002 |
| test_単体正常系_get_loggerが_設定レベル未満のメッセージを受け取った場合_出力から除外する[50-40] | 成功 | 0.002 |
...
| test_単体正常系_build_file_handlerが_ローテーション有効の場合_日次ローテーションのハンドラを構築する | 成功 | 0.001 |
| test_単体正常系_build_file_handlerが_ローテーション無効の場合_追記モードの単一ファイルハンドラを構築する | 成功 | 0.001 |
...
| test_結合_保持世代数を超える退避ファイルが事前配置されている場合_古いものから削除される | 成功 | 0.003 |
| test_結合_複数モジュールが単一ファイルへ統合された構成で_日付変更後も新しい当日ファイルにのみ記録される | 成功 | 0.003 |
| test_結合_rotation_enabledがfalseの場合_ローテーションが発生せず単一ファイルへ追記され続ける | 成功 | 0.002 |
| test_結合_retention_daysが設定された場合_ファイルハンドラのbackupCountへ反映される | 成功 | 0.001 |
| test_結合_ロガー個別ファイル設定でも同一のローテーション動作が適用される | 成功 | 0.002 |
...
| test_結合_pre_commit_config_yamlのフックentryが_pdm_run_pytestを疑似成功コマンドに置換した場合_reports_test_evidence_mdをステージする | 成功 | 0.049 |
...
| test_単体正常系_TestCaseResultが_必須項目を受け取った場合_各属性を保持する | 成功 | 0.001 |
...
| test_異常系_to_timezoneが_未知のタイムゾーン名を指定された場合_InvalidTimezoneErrorを送出する | 成功 | 0.000 |
...
| test_単体正常系_parse_datetimeが_enumにないstrptime書式文字列を指定された場合_その書式で解析する | 成功 | 0.001 |
