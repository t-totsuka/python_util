...
- パッケージバージョンは日付ベースの `{YYYYMM}.{D}.{sequence}` 形式で運用する（`pyproject.toml` の `version`）
  - `YYYYMM`: リリース年月（例: `202607`）
  - `D`: リリース日（月内の日。PEP 440 では数値セグメントの先頭ゼロが正規化されるため、ゼロ埋めしない: 7月5日 → `5`）
  - `sequence`: 同日内のリリース連番（`1` 始まり）
  - 例: 2026-07-15 の最初のリリース → `202607.15.1`、同日2回目 → `202607.15.2`
...
_2026-07-15: バージョニング規約（`{YYYYMM}.{D}.{sequence}`）を追加。_
